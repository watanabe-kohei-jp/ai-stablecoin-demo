...
### 発展2：流動性の階段（SC ⇄ 預金Vault ⇄ ST・償還ラグの実証）

自動スイープを **3層** に拡張。「置き場」に奥行きを持たせ、本物のRWAが持つ**償還レイテンシ
（流動性ミスマッチ）** をコードで実証する。

```
手前: USDC（決済用・利回りなし）
  ⇅ JIT償還（即時）
中間: 預金Vault = MockYieldVault を低APYで流用（模擬トークン化預金・即時償還）
  ⇅ 持ち替え
奥  : MockSecurityToken（模擬ST・高APY・二段階償還 = requestRedeem → ラグ → claimRedeem）
```

- **JITは即時層（預金Vault）からのみ**。STは償還ラグ中は間に合わない → wallet は支払いを**見送る**
  （「24/7の約束 vs 原資産はT+N」という流動性ミスマッチの再現）。
- 奥の資金を使うには **前もっての償還予約（`plan_liquidity`）→ ラグ経過 → 受取（`claim_st`）** が必要。
  この流動性計画も brain ではなく **wallet（信頼境界）** の仕事。

```powershell
python scripts\deploy_ladder.py --compile-only            # コンパイル検証（ガス不要）
python scripts\deploy_ladder.py --deploy-deposit --deploy-st   # 2契約をデプロイ（.envに自動記録）
python scripts\deploy_ladder.py --seed-deposit 1 --seed-st 1   # 利回り原資をシード
# 売り手サーバー起動後：
python scripts\ladder_demo.py   # 通し：3層sweep→JIT→ミスマッチで見送り→償還予約→受取→再挑戦成功
```
...
| `contracts/MockSecurityToken.sol` | 模擬ST（高APY・**二段階償還**＝償還ラグ。流動性ミスマッチの実証用） |
| `scripts/deploy_vault.py` | Vault のコンパイル/デプロイ/準備金シード（`--compile-only`/`--deploy`/`--seed`） |
| `scripts/deploy_ladder.py` | 流動性の階段（預金Vault＋ST）のコンパイル/デプロイ/シード |
| `scripts/ladder_demo.py` | 3層通しデモ（sweep→JIT→ミスマッチ見送り→償還予約→受取→再挑戦） |
